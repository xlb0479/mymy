...
默认情况下kubelet的`--register-node`选项为true，此时kubelet会把自己注册到API服务上。大部分情况下都是如此。

自注册时，kubelet使用如下参数启动：

- `--kubeconfig`——向apiserver提供认证凭证的证书路径。
- `--cloud-provider`——如何从云服务商那边获取元数据。
- `--register-node`——自动注册到API服务上。
- `--register-with-taints`——注册时需要添加的节点的冷屁股（逗号间隔的`<key>=<value>:<effect>`），此选项只在`register-node`为true时有效。
- `--node-ip`——节点IP。
- `--node-labels`——注册时需要添加的标签（从1.13版本开始，Admission控制器对节点标签有一些强制限制，[详见这里]()）。
- `--node-status-update-frequency`——kubelet向主节点上报状态数据的频率。

如果启用了[节点授权模式]()和Admission控制器，kubelet就只有权限创建、修改自身节点的资源。

#### 手动管理节点

集群管理员可以创建、修改节点对象。

如果管理员要手动创建节点对象，就要设置`--register-node=false`。

不论`--register-node`设置成啥，管理员都是能去修改节点资源的。包括给节点设置标签、关闭调度能力等。

通过节点上的标签以及pod上的标签选择器，就可以来控制pod的调度，比如让pod只运行在一部分节点上。

如果关闭了节点的调度能力，就不会再有新的pod被指派到这个节点上，但是不会影响那些已经存在于该节点的pod。这样有个啥好处呢，比如我们想重启某个节点，就可以先关闭它的调度能力，作为准备工作的一部分。通过如下命令来关闭节点的调度能力：

```text
kubectl cordon $NODENAME
```

>**注意**：由DaemonSet控制器创建的Pod会绕过k8s的调度器，也就不会鸟你节点是否关闭了调度能力。就是说即便我们现在要重启机器了，上面的应用也都清了，但是对于Daemon来说，是属于这个节点机器的，归机器自己来管。

>**小心**：如果用`kubectl cordon`命令关闭了节点的调度能力，有一个副作用，此时Service控制器会把这个节点从LoadBalancer中移除，相当于直接掐掉了这些节点的入口流量。

### 节点容量

节点的容量（CPU个数和内存数量）是节点对象的一部分。一般来说，创建了节点对象之后，节点自注册，上报节点容量。如果你是[手动管理节点](#手动管理节点)，那添加节点的时候就要设置容量信息。

k8s的调度器会确保一个节点上的所有Pod都能拥有足够的资源。它会检查节点上所有容器需求的资源总量，看看是否超过了节点的容量。这里只是针对由kubelet启动的容器，不包含直接通过[容器运行时](../概要/Kubernetes组成.md#容器运行时container-runtime)启动或非容器的进程。

如果想给那些非Pod进程保留一些系统资源，请移步[为系统进程保留资源]()。

## 节点拓扑

**功能状态**：`Kubernetes v1.18`（测试阶段）

如果你打开了`TopologyManager`[特性门]()（难道是门事件？），kubelet在进行资源规划的时候就可以利用拓扑提示了。

## API对象

对k8s的REST API来说，节点属于顶级资源。详见[节点API对象]()。

## 接下来……

- 阅读[节点组成](../概要/Kubernetes组成.md#Node)
- 学学节点层面的拓扑：[控制节点拓扑管理策略]()